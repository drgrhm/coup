...
if __name__ == "__main__":
    ...
    data_save_path = "dat/many_experiment_{}_{}_{}_{}.p".format(args.seed, args.dataseed, u_str, args.dataset)
    ...
    try: 
        ...
        data = {}
        data['coup'] = coup(env, lambda t: u_fn(t, **u_params), args.delta, epsilon_fn, gamma_fn, max_phases=num_phases, n_max=env.num_configs, m_max=env.num_instances, doubling_condition="new")
        env.reset()
        ...
        for p in range(num_phases):
            ...
            d = oup(env, lambda t: u_fn(t, **u_params), args.delta, epsilon_min=epsilon, n=num_configs, m_max=env.num_instances, doubling_condition="new")
            data['oup'].append(d)
            env.reset()
    ...
    total_time_per_phase_oup = [data['oup'][p]['total_time'][-1] for p in range(num_phases)]
    ...
    plt.ylim(0, 1)
    plt.xscale('log')
    plt.legend(fontsize=fs['legend'], loc='upper right')
    ...
    plt.savefig("img/many_experiment_total_times_{}_{}_{}_{}.pdf".format(args.seed, args.dataseed, u_str, args.dataset), bbox_inches='tight')
    plt.clf()

    
    plt.scatter(data['utilities'], data['oup'][-1]['total_times'][-1], label="OUP", c=colors[2])
    plt.scatter(data['utilities'], data['coup']['phase'][-1]['total_times'], label="COUP", c=colors[3])
    # plt.ylim(9e2, 5e6)
    plt.xticks(fontsize=fs['ticks'])
    plt.yticks(fontsize=fs['ticks'])
    ...
    plt.savefig("img/many_experiment_time_per_config_{}_{}_{}_{}.pdf".format(args.seed, args.dataseed, u_str, args.dataset), bbox_inches='tight')
    plt.clf()
