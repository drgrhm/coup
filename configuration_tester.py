...
class ConfigurationTester():
    ...
    def execute_step(self, env, t):
        ...
        elapsed = env.run(self.cid, l, self.theta)

        # print(elapsed, self.theta)

        if elapsed >= self.theta: # timeout
            rt = self.theta
            self.Q.appendleft((l, self.theta_multiplier * self.theta))
            # print("APPENDING {}, len(Q)={}, q={}".format(self.theta_multiplier * self.theta, len(self.Q), self.q))
        else:
            rt = elapsed
        ...
        return rt, self.lcb, len(self.instance_runtimes_capped)
